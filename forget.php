...
if(isset($_GET['key']) && isset($_GET['email'])) {
    $key=$_GET['key'];
    $email=$_GET['email'];
    $check=mysqli_query($dbconfig,"SELECT * FROM tblusers WHERE email='$email' and pasword='$pasword'");
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if ($password2==$password1) {
            ...
            mysqli_query($dbconfig,"DELETE FROM tblusers where email='$email' and pasword='$pasword'");

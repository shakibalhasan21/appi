...
<body>
    <div class="container mt-5">
        <h2 class="text-center">Upload SIGN COPY and Extract Data</h2>
        <div class="card p-4">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit" name="upload" class="btn btn-primary">Extract Data</button>
            </form>
        </div>
        ...
        <div class="mt-4">
            ...
            if (isset($_POST['upload']) && isset($_FILES['pdfFile'])) {
                ...
                try {
                    ...
                    file_put_contents('extracted_text.txt', $textContent);
                    ...
                    $newFormat = [
                        'name' => $finalData['Name(Bangla)'] ?? 'তথ্য পাওয়া যায়নি',
                        'name_en' => $finalData['Name(English)'] ?? 'তথ্য পাওয়া যায়নি',
                        'gender' => $finalData['Gender'] ?? 'তথ্য পাওয়া যায়নি',
                        'bloodGroup' => $finalData['Blood Group'] ?? 'তথ্য পাওয়া যায়নি',
                        'date_of_birth' => $finalData['Date of Birth'] ?? 'তথ্য পাওয়া যায়নি',
                        'father' => $finalData['Father Name'] ?? 'তথ্য পাওয়া যায়নি',
                        'mother' => $finalData['Mother Name'] ?? 'তথ্য পাওয়া যায়নি',
                        'spouse' => $finalData['Spouse Name'] ?? 'তথ্য পাওয়া যায়নি',
                        'national_id' => $finalData['National ID'] ?? 'তথ্য পাওয়া যায়নি',
                        'pin' => $finalData['Pin'] ?? 'তথ্য পাওয়া যায়নি',
                        'voter_no' => $finalData['Voter No'] ?? 'তথ্য পাওয়া যায়নি',
                        'birth_place' => $finalData['Birth Place'] ?? 'তথ্য পাওয়া যায়নি',
                        'permanent_address' => $finalData['Permanent Address'] ?? 'তথ্য পাওয়া যায়নি',
                        'present_address' => $finalData['Present Address'] ?? 'তথ্য পাওয়া যায়নি',
                    ];

                    echo "<h4 class='mt-3'>Extracted Data From (SIGN):</h4>";
                    echo "<pre>" . json_encode($newFormat, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) . "</pre>";
...
<head>
   <title>SERVER SEBA ONLINE-<?php echo isset($newFormat['nid']) ? $newFormat['nid'] : ''; ?></title>

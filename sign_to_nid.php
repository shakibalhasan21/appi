<?php
session_start();
require 'pdfcode/vendor/autoload.php';
...
if (!isset($_SESSION['uid'])) {
   ...
   if (isset($_POST['submit'])) {
      ...
      if($obj->is_premium($user_id) != 1){
         if ($diff >= $sign_to_nid) {
            $withdraw = $obj->get_withdraw($user_id, $sign_to_nid);
            $showFrom = false;
         } else {
...
<?php
// Check if form is submitted with a PDF file
if (isset($_POST['upload']) && isset($_FILES['pdfFile'])) {

    // Function to clean address from text
    function cleanAddress($addressText) {
        if (!$addressText) return null;
        $lines = explode("\n", trim($addressText));
        $keys = ['Division', 'District', 'Upozila', 'Union/Ward', 'Mouza/Moholla', 'Village/Road', 'Post Office', 'Postal Code', 'Region', 'RMO'];
        $cleaned = [];
        foreach ($lines as $line) {
            foreach ($keys as $key) {
                if (strpos($line, $key) !== false) {
                    preg_match("/$key\s*[:\s]*(.*?)(\t|$)/u", $line, $matches);
                    if (!empty($matches[1])) {
                        $cleaned[$key] = trim($matches[1]);
                    }
                }
            }
        }
    ...
    function formatAddress($address) {
        if (!$address) return "ঠিকানা: তথ্য পাওয়া যায়নি";
        $formattedAddress = "";
        $formattedAddress .= isset($address['Mouza/Moholla']) ? "বাসা/হোল্ডিং: {$address['Mouza/Moholla']}, " : '';
        $formattedAddress .= isset($address['Village/Road']) && $address['Village/Road'] !== $address['Mouza/Moholla'] ? "গ্রাম/রাস্তা: {$address['Village/Road']}, " : '';
        $formattedAddress .= isset($address['Post Office']) ? "ডাকঘর: {$address['Post Office']}-{$address['Postal Code']}, " : '';
        $formattedAddress .= isset($address['Upozila']) ? "{$address['Upozila']}, " : '';
        $formattedAddress .= isset($address['District']) ? "{$address['District']}, " : '';
        $formattedAddress .= isset($address['Region']) ? "{$address['Region']}" : '';
        return rtrim($formattedAddress, ', ') ?: "ঠিকানা: তথ্য পাওয়া যায়নি";
    }
    ...
    try {
        ...
        file_put_contents($uploadDir . 'extracted_text.txt', $textContent);

        // ---- IMAGE EXTRACTION ----
        // We'll use pdfimages (poppler-utils) to extract images
        // First, save the uploaded file to a known location
        $tempPdfPath = $uploadDir . uniqid() . '.pdf';
        move_uploaded_file($file, $tempPdfPath);
        
        // Create a unique folder for this PDF's images
        $uniqueImageDir = $imageDir . uniqid() . '/';
        if (!file_exists($uniqueImageDir)) {
            mkdir($uniqueImageDir, 0777, true);
        }
        
        // Extract images using pdfimages - a command line tool
        // -j flag preserves JPEG images
        // -png converts non-JPEG images to PNG
        // The output prefix will be followed by a number for each image
        $outputPrefix = $uniqueImageDir . 'image';
        $command = "pdfimages -j -png \"$tempPdfPath\" \"$outputPrefix\"";
        exec($command, $output, $returnCode);
        
        // Check if extraction was successful
        if ($returnCode !== 0) {
            throw new Exception("Image extraction failed. Make sure poppler-utils is installed.");
        }
        
        // Get all extracted images
        $extractedImages = glob($uniqueImageDir . 'image-*');
        
        // Identify photo and signature based on naming convention or other heuristics
        // For this example, we'll assume:
        // - The photo is typically larger in dimensions than the signature
        // - The photo usually comes before the signature in the PDF
        ...
        if (count($extractedImages) >= 2) {
            ...
            usort($extractedImages, function($a, $b) {
                $numA = (int)preg_replace('/.*-(\d+)\..*/', '$1', $a);
                $numB = (int)preg_replace('/.*-(\d+)\..*/', '$1', $b);
                return $numA - $numB;
            });
            
            // Get image sizes to differentiate photo from signature
            $imageSizes = [];
            foreach ($extractedImages as $img) {
                $size = getimagesize($img);
                $imageSizes[$img] = $size[0] * $size[1]; // area in pixels
            }
            
            // Sort by size (descending)
            arsort($imageSizes);
            $sortedImages = array_keys($imageSizes);
            
            // Usually the photo is larger than the signature
            if (count($sortedImages) >= 2) {
                // Copy files to final destinations with proper names
                $photoPath = $imageDir . $fileName . '_photo.' . pathinfo($sortedImages[0], PATHINFO_EXTENSION);
                $signPath = $imageDir . $fileName . '_signature.' . pathinfo($sortedImages[1], PATHINFO_EXTENSION);
                
                copy($sortedImages[0], $photoPath);
                copy($sortedImages[1], $signPath);
            } elseif (count($sortedImages) == 1) {
                // If only one image is found, assume it's the photo
                $photoPath = $imageDir . $fileName . '_photo.' . pathinfo($sortedImages[0], PATHINFO_EXTENSION);
                copy($sortedImages[0], $photoPath);
                $signPath = null;
            }
        ...
        $fields = ['National ID', 'Pin', 'Voter No', 'Name\\(Bangla\\)', 'Name\\(English\\)', 'Date of Birth', 'Birth Place', 'Father Name', 'Mother Name', 'Spouse Name', 'Gender', 'Occupation', 'Blood Group'];
        $data1 = [];
        foreach ($fields as $field) {
            preg_match("/$field\s*[:\s]*(.*?)(\n|$)/u", $textContent, $matches);
            $data1[trim(str_replace('\\', '', $field))] = trim($matches[1] ?? '');
        }

        preg_match("/Present Address\s*[:\s]*(.*?)(Permanent Address|$)/su", $textContent, $presentAddressMatches);
        preg_match("/Permanent Address\s*[:\s]*(.*?)(Foreign Address|$)/su", $textContent, $permanentAddressMatches);

        $presentAddress = $presentAddressMatches[1] ?? null;
        $permanentAddress = $permanentAddressMatches[1] ?? null;

        $data2 = [
            'Present Address' => formatAddress(cleanAddress($presentAddress)),
            'Permanent Address' => formatAddress(cleanAddress($permanentAddress)),
        ];
        ...
        foreach ($finalData as $key => $value) {
            if (empty($value)) {
                $finalData[$key] = strpos($key, 'Address') !== false ? "ঠিকানা: তথ্য পাওয়া যায়নি" : "তথ্য পাওয়া যায়নি";
            }
        }

        // Final formatted array with photo and signature paths
        $newFormat = [
            'name' => $finalData['Name(Bangla)'],
            'name_en' => $finalData['Name(English)'],
            'gender' => $finalData['Gender'],
            'bloodGroup' => $finalData['Blood Group'],
            'date_of_birth' => $finalData['Date of Birth'],
            'father' => $finalData['Father Name'],
            'mother' => $finalData['Mother Name'],
            'spouse' => $finalData['Spouse Name'],
            'national_id' => $finalData['National ID'],
            'pin' => $finalData['Pin'],
            'voter_no' => $finalData['Voter No'],
            'birth_place' => $finalData['Birth Place'],
            'permanent_address' => $finalData['Permanent Address'],
            'present_address' => $finalData['Present Address'],
            'photo' => $photoPath,
            'signature' => $signPath,
        ];

        $success_message = "তথ্য ও ছবি সফলভাবে প্রাপ্ত হয়েছে।";
...
<?php if ($showFrom) { ?>
      ...
      <main id="main" class="main">
         <div id="inp" class="container card px-3 py-5 mt-6 col-md-12 mb-5">
            ...
            <div class="container mt-5">
               <h2 class="text-center">UPLOAD SIGN COPY</h2>
               <div class="card p-4">
                     <form action="" method="POST" enctype="multipart/form-data">
                        <div class="mb-3 drop-zone" id="dropZone">
                           <label for="pdfFile" class="form-label">Upload File</label>
                           <input type="file" name="pdfFile" id="pdfFile" class="form-control d-none" accept=".pdf" required>
                        </div>
                        <div style="text-align: center;">
                           <button type="submit" name="upload" class="btn btn-outline-primary">Extract Data</button>
                        </div>
                     </form>
               </div>
            </div>
            <form action="" method="POST">
                <?php if(isset($photoPath) && file_exists($photoPath)): ?>
                  <div class="row text-center">
                     <div class="mb-3 myDiv" id="showOne">
                        ...
                        <?php if (!empty($newFormat['photo'])): ?>
                           <img src="<?php echo $newFormat['photo']; ?>" alt="Photo" style="max-width: 120px; margin-bottom:10px;"><br>
                        <?php endif; ?>
                        <input type="hidden" name="photo_path" value="<?php echo $newFormat['photo']; ?>">
                     </div>
                     <div class="mb-3 myDiv" id="showOne">
                        <label>Signature:</label><br>
                        <?php if (!empty($newFormat['signature'])): ?>
                           <img src="<?php echo $newFormat['signature']; ?>" alt="Signature" style="max-width: 120px; margin-bottom:10px;"><br>
                        <?php endif; ?>
                        <input type="hidden" name="signature_path" value="<?php echo $newFormat['signature']; ?>">
                     </div>
                  </div>
                <?php endif; ?>
               <div class="row">
                  <div class="mb-3 myDiv" id="showOne">
                     ...
                     <input type="text" class="form-control" name="name_bn" value="<?php echo $newFormat['name'] ?>" placeholder="নাম (বাংলা)" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Name (English):</label>
                     <input type="text" class="form-control" name="name_en" value="<?php echo $newFormat['name_en'] ?>" placeholder="Name (English)" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>NID Number:</label>
                     <input type="text" class="form-control" name="nid" value="<?php echo $newFormat['national_id'] ?>" placeholder="825218****" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>PIN Number:</label>
                     <input type="text" class="form-control" name="pin" value="<?php echo $newFormat['pin'] ?>" placeholder="PIN Number" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Father's Name (Bangla):</label>
                     <input type="text" class="form-control" name="father" value="<?php echo $newFormat['father'] ?>" placeholder="Father's Name (বাংলা)" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Mother's Name (Bangla):</label>
                     <input type="text" class="form-control" name="mother" value="<?php echo $newFormat['mother'] ?>" placeholder="Mother's Name (বাংলা)" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Birth Place:</label>
                     <input type="text" class="form-control" name="birth" value="<?php echo $newFormat['birth_place'] ?>" placeholder="Birth Place" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Date Of Birth:</label>
                     <input type="text" class="form-control" name="dob" value="<?php
                        $date = new DateTime($newFormat['date_of_birth']);
                        $formatted_date = $date->format('d M Y'); // Format to '11 Nov 1997'
                        echo isset($newFormat['date_of_birth']) ? htmlspecialchars($formatted_date) : '';
                        ?>" placeholder="05 Nov 2005" required>
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Blood Group:</label>
                     <input type="text" class="form-control" name="blood" value="<?php echo $newFormat['bloodGroup'] ?>" placeholder="Blood Group" >
                  </div>
                  <div class="mb-3 myDiv" id="showOne">
                     <label>Address:</label>
                     <input type="text" class="form-control" name="address" value="<?php echo $newFormat['present_address'] ?>" placeholder="Address Line" required>
                  </div>
               </div>

               <div class="text-center">
                     <span style="font-size: 12px;padding: 10px;margin: auto;text-align: center; color: red;">
                        <b>Note:</b> আপনার ফাইলের জন্য <?php echo $sign_to_nid ?> টাকা চার্জ কাটা হবে.
                     </span>
               </div>
               <br>
               <div style="text-align: center;">
               ...
               <!-- <input type="submit" name="submit" class="btn btn-danger" onclick="submit()" style="font-weight: bold;" value="Submit" /> -->
            </form>
   ...
   <?php } else { ?>
      ...
      <head>
         <title>SERVER SEBA ONLINE-<?php echo isset($_POST['nid']) ? $_POST['nid'] : ''; ?></title>
                                       ...
                                       <div class="pt-[3.8px] pr-1 pl-[2px] bg-center w-full flex justify-between gap-x-2 pb-5 relative">
                                          ...
                                          <div class="relative z-50">
                                             <label for="photo" class="custom-file-upload">
                                                ...
                                                <img style="margin-top:-2px" id="userPhoto" class="w-[68.2px] h-[78px]" alt="photo" src="<?php echo isset($_POST['photo_path']) ? htmlspecialchars($_POST['photo_path']) : 'photo/default.jpg'; ?>" />
                                             ...
                                             <div class="text-center text-xs flex items-start justify-center pt-[5px] w-[68.2px] mx-auto h-[38.5px] overflow-hidden" id="card_signature"><span style="font-family:Comic sans ms"></span>
                                                <label for="userSignUpload" class="custom-file-upload">
                                                   ...
                                                   <img id="sign" src="<?php echo isset($_POST['signature_path']) ? htmlspecialchars($_POST['signature_path']) : 'photo/default.jpg'; ?>" alt="sign" />
                                                </label>
                                    ...
                                    <div id="nid_back" class="w-full border-[1.999px] border-[#000]">
                                       ...
                                       <div class="py-1 pl-2 pr-1">
                                          ...
                                          <div class="flex justify-between items-center -mt-[5px]">
                                             ...
                                             <span class="pr-4 bn" style="font-size:12px;padding-top:1px">প্রদানের তারিখ:<span class="ml-2.5" id="card_date"></span>
                                             <?php
